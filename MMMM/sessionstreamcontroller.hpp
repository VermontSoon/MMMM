...
public:
    ...
    SessionStreamController(std::map<basefw::ID, uint64_t>& sessionRTTs,
    std::map<basefw::ID, uint64_t>& sessioncwnds):
            ...
            std::weak_ptr<SessionStreamCtlHandler> ssStreamHandler)
    {
        ...
        m_congestionCtl.reset(new BALIACongestionContrl(m_ccConfig, m_sessionId, session_RTTs, session_cwnds));
        ...
        session_cwnds[sessionId]=2;
        session_RTTs[sessionId]=200;
    ...
    void OnDataPktReceived(uint32_t seq, int32_t datapiece, Timepoint recvtic)
    {
        ...
        if (inFlight)
        {
            ...
            session_RTTs[m_sessionId]=m_rttstats.SmoothedOrInitialRtt().ToMilliseconds();
    ...
    Duration GetRtt()
    {
        Duration rtt{ Duration::FromMicroseconds(0) };
        if (isRunning)
        {
